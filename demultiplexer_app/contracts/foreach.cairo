...
@storage_var
func my_data(index : felt) -> (delay : felt):
end
...
func set_transaction_delays{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    ...
    my_data.write(0, 1234)
    my_data.write(1, 3456)
    my_data.write(2, 7890)
    my_data.write(3, 5321)
    my_data.write(4, 4567)
...
func array_loop1{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(size) -> (
    res : felt
):
    alloc_locals
    ...
    if size == 0:
        ...
        let (_data) = my_data.read(_index)
        recipients_transactions_delay.write(_index, _data)
        index.write(_index + 1)
    end
    let (_result) = array_loop1(size=size - 1)
