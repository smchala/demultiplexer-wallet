...
class StarknetContract:
    ...
    def _build_contract_function(self, function_abi: dict) -> Callable:
        ...
        arg_names, arg_types = parse_arguments(
            arguments_abi=function_abi["inputs"])
        ...
        func = types.FunctionType(
            code=func_code, globals=globals(), closure=closure)
        func.__annotations__ = {**calldata_annotations,
                                "return": tuple(retdata_annotations)}
    ...
    def _get_annotation(self, arg_type: CairoType, is_nested: bool = False) -> PythonType:
        ...
        if isinstance(arg_type, TypePointer):
            ...
            pointee_type = self._get_annotation(
                arg_type=arg_type.pointee, is_nested=True)
        ...
        if isinstance(arg_type, TypeStruct):
            struct_def = self.struct_manager.get_struct_definition(
                name=arg_type.scope.path[-1])
            return Tuple[
                tuple(
                    self._get_annotation(
                        arg_type=member.cairo_type, is_nested=True)
    ...
    def _build_function_call(
        ...
        for input_entry in function_abi["inputs"]:
            ...
            arg_cairo_type = mark_type_resolved(
                parse_type(code=input_entry["type"]))
            if name not in args:
                continue
        ...
        return StarknetContractFunctionInvocation(
            ...
            retdata_arg_types=retdata_arg_types,
            retdata_tuple=namedtuple(
                f"{function_name}_return_type", retdata_arg_names),
...
class StarknetContractFunctionInvocation:
    ...
    async def _invoke_on_given_state(
        ...
        execution_info = await state.invoke_raw(
            ...
            max_fee=max_fee,
            signature=None if signature is None else cast_to_felts(
                values=signature),
        )
        ...
        return StarknetTransactionExecutionInfo.from_internal(
            ...
            result=result,
            main_call_events=self._build_events(
        ...
        for raw_event in raw_events:
            ...
            try:
                args = self._build_arguments(
                    ...
                    arg_types=self.event_manager.get_event_argument_types(
                        identifier=selector),
                )
                args_dataclass = self.event_manager.get_contract_event(
                    identifier=selector)
    ...
    def _build_arguments(self, arg_values: List[int], arg_types: List[CairoType]) -> List[Any]:
        ...
        def build_arg(
            ...
            if isinstance(arg_type, TypeTuple):
                return tuple(
                    build_arg(arg_type=cairo_type,
                              arg_value_iterator=arg_value_iterator)
                    for cairo_type in arg_type.types
                )
            if isinstance(arg_type, TypeStruct):
                ...
                struct_def = self.struct_manager.get_struct_definition(
                    name=struct_name)
                contract_struct = self.struct_manager.get_contract_struct(
                    name=struct_name)
                return contract_struct(
                    *(
                        build_arg(arg_type=member.cairo_type,
                                  arg_value_iterator=arg_value_iterator)
                        for member in struct_def.members.values()
                    )
                )
            if isinstance(arg_type, TypePointer):
                ...
                return [
                    build_arg(arg_type=arg_type.pointee,
                              arg_value_iterator=arg_value_iterator)
