...
MALICIOUS_ADDRESS = 0x12341234
YAGI_ROUTER_GOERLI_ADDRESS = 0x0221b6a7865025ca17a24eb7ab1c9423fffe8a92c61311e6b2e420239606cd6d
# # ////////////////////////////////////////////////////////////////////////////////
...
@pytest.mark.asyncio
async def test_is_transaction_ready():
    """Test is_transaction_ready method, check if transactions are ready. """
    starknet = await Starknet.empty()
...
# @pytest.mark.asyncio
# async def test_probe_demux_transfers():
#     """Test probe_demux_transfers method, check if transactions are ready. """
#     starknet = await Starknet.empty()
...
#     update_starknet_block(starknet=starknet, block_timestamp=1666672635)
#     # set 1st recipient
#     await TestData.set_recipient1(contract).invoke()
...
# # Fail path
#     # Make sure if we call probe_demux_transfers from a malicious address, not from yagi router we should fail
#     with pytest.raises(StarkException) as excinfo:
#         await contract.probe_demux_transfers().invoke(caller_address=MALICIOUS_ADDRESS)
#     assert "ASSERT_EQ instruction failed: 305402420 != 964192284595246656383835570063451809862878018693587543515332983943688801645" in str(
#         excinfo.value)
#     # 305402420 decimal to hex 12341234 our MALICIOUS_ADDRESS
#     # 964192284595246656383835570063451809862878018693587543515332983943688801645 decimal to hex 221B6A7865025CA17A24EB7AB1C9423FFFE8A92C61311E6B2E420239606CD6D our YAGI_ROUTER_GOERLI_ADDRESS

# # Happy path
#     # As the first recipients transaction delay has passed compared to out mock block time, the probe_demux_transfers
#     # should return 1
#     execution_info = await contract.probe_demux_transfers().invoke(caller_address=YAGI_ROUTER_GOERLI_ADDRESS)
#     assert execution_info.result[0] == 1

#     # we should increment the index and now we should have 0 as the date of the 2nd recipient is still in the
#     # future compared to our block time (mocked)
#     execution_info = await contract.probe_demux_transfers().invoke(caller_address=YAGI_ROUTER_GOERLI_ADDRESS)
#     assert execution_info.result[0] == 0

#     # if we change the block time to be in future comparedd to the transaction delay set for the second recipient
#     # probe_demux_transfers should return 1
#     update_starknet_block(starknet=starknet, block_timestamp=1696672635)
#     execution_info = await contract.probe_demux_transfers().invoke(caller_address=YAGI_ROUTER_GOERLI_ADDRESS)
#     assert execution_info.result[0] == 1
